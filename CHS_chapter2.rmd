...
#### 2.1.1 Estimate non-parametrically the cumulative hazards of death for subjects with or without ESVEA.
...
### 2.1.2 Make a non-parametric test for comparison of the two.
...
### 2.1.3 Make a similar analysis based on a model where the hazard is assumed constant within 5-year intervals.
...
### 2.2.1 Make a version of the data set enabling an analysis of the composite endpoint of stroke or death without stroke (‘stroke-free survival’, i.e. define the relevant Time and Status variables), see Section 1.2.4.
...
### 2.2.2 Estimate non-parametrically the cumulative hazards of stroke-free survival for subjects with or without ESVEA.
...
### 2.2.3 Make a non-parametric test for comparison of the two.

Then the hazards are compared using a logrank test
...
### 2.2.4 Make a similar analysis based on a model where the hazard is assumed constant within 5-year intervals.
...
## Exercise 2.3 Consider the data from the Copenhagen Holter study and the composite end-point stroke-free survival.

### 2.3.1 Fit a Cox model and estimate the hazard ratio between subjects with or without ESVEA.
...
### 2.3.2 Fit a Poisson model where the hazard is assumed constant within 5-year intervals and estimate the hazard ratio between subjects with or without ESVEA.
...
### 2.3.3 Compare the results from the two models
...
## Exercise 2.4 Consider the data from the Copenhagen Holter study and the composite end-point stroke-free survival.

### 2.4.1 Fit a Cox model and estimate the hazard ratio between subjects with or without ESVEA, now also adjusting for sex, age, and systolic blood pressure (sysBP).
...
### 2.4.2 Fit a Poisson model where the hazard is assumed constant within 5-year intervals and estimate the hazard ratio between subjects with or without ESVEA, now also adjusting for sex, age, and sysBP.
...
### 2.4.3 Compare the results from the two models.
...
### 2.5.1 Check the Cox model from the previous exercise by examining proportional hazards between subjects with or without ESVEA and between men and women.
...
### 2.5.2 Check for linearity on the log(hazard)-scale for age and sysBP
...
### 2.5.3 Do the same for the Poisson model
...
## Exercise 2.6 Consider the data from the Copenhagen Holter study and focus now on the mortality rate after stroke

### 2.6.1 Estimate non-parametrically the cumulative hazards for subjects with or without ESVEA using the time variable ‘time since recruitment’.
...
### 2.6.2 Assume proportional hazards and estimate the hazard ratio between subjects with or without ESVEA.
...
### 2.6.3 Repeat these two questions using now the time variable ‘time since stroke’ and compare the results.
...
### 2.7.1 Consider the data from the Copenhagen Holter study and fit Cox models for the cause-specific hazards for the outcomes stroke and death without stroke including ESVEA, sex, age, and sysBP.
...
### 2.7.1 Compare with the results from Exercise 4 in Chapter 2 (first question).
