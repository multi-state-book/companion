...
twoYr <- summary(kmfit,times=2*365.25)
#difference in survival at 3 years between edema=0 and edemo=1 (for example) is
twoYr$surv[1] - twoYr$surv[2]
# SD
diffSD <- sqrt(twoYr$std.err[2]^2 + twoYr$std.err[1]^2)
# 95% CI for the 
c(twoYr$surv[1] - twoYr$surv[2] - 1.96 *diffSD, twoYr$surv[1] - twoYr$surv[2] + 1.96 *diffSD)
# z-test test statistic
zStat <- (twoYr$surv[1] - twoYr$surv[2])/diffSD
# two-sided p-value
...
# Death w/o transplant
cifreg(Event(days,status)~tment+alb+log2bili+sex+age, cause=2, 
       data = subset(pbc3, !is.na(alb)),propodds=NULL)
...
# Create Figure
fig4.23 <- ggplot(aes(x = time / 365.25, y = surv, linetype = type), 
                 data = pdata) + 
  geom_step(linewidth = 1) + 
...
coxph(Surv(days, status == 0) ~ tment, data = pbc3, method = "breslow")
coxph(Surv(days, status == 0) ~ alb, data = pbc3, method = "breslow")
coxph(Surv(days, status == 0) ~ bili, data = pbc3, method = "breslow")
```
